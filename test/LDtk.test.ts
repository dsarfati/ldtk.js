import { LDtk } from '../src'

import testLDtk from './files/test.json'
...
// describe('importer', () => {
//   it('full API', () => {
//     // Compares official Full API sample data with the processed data.
...
//       zip(o_enum.values, p_enum.values).forEach(
//         ([o, p]) => (
...
//       zip(o_level.__neighbours, p_level.neighbours).forEach(
//         ([o, p]) => (expect(o?.dir).toEqual(p?.dir), expect(o?.levelUid).toEqual(p?.level.uid)),
//       )
...
describe('LDtk', () => {
  test('fromJSON', () => {
    let root = LDtk.fromJSON(testLDtk)
